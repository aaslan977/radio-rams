...
export default defineConfig({
  plugins: [
    ...
    VitePWA({
      ...
      // og-image.jpg сюда намеренно не добавлен и в precache не попадает:
      // его забирают только краулеры соцсетей обычным запросом по абсолютному
      // URL, а живому пользователю он не показывается ни разу. Класть ~400 КБ
      // в офлайн-кэш ради этого незачем — отсутствие файла в списке precache
      // не недосмотр.
